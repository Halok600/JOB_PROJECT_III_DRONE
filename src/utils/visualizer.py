...
class TrackVisualizer:
    ...
    def draw(
        ...
        # --- Step1: Draw ALL trails on a single black overlay ---
        ...
        # Vlack pixels (0,0,0) add nothing; trail pixels light up.
        ...
        for track in tracks:
            ...
            cv2.rectangle(out,(lx,ly),(lx+tw_l+4,ly+th_l+5),color,-1)

            cv2.putText(out, label, (lx+2,ly+th_l+1),cv2.FONT_HERSHEY_SIMPLEX,0.48,(255,255,255),1, cv2.LINE_AA)
    ...
    def _draw_hud(self, frame: np.ndarray, fps: float, frame_id: int, n_tracks: int) -> None:
        ...
        frame[:bar_h,:] = roi
        fps_color = (
            (0,210,0) if fps>=15 else
            (0,190,210) if fps >= 8.0 else
            (0,0,210)
        )
        cv2.putText(frame,f"FPS: {fps:.1f}",(6,20), cv2.FONT_HERSHEY_SIMPLEX, 0.58, fps_color,2,cv2.LINE_AA)
        cv2.putText(frame,f"Frame: {frame_id:05d}", (125,20), cv2.FONT_HERSHEY_SIMPLEX, 0.52, (200,200,200), 1, cv2.LINE_AA)
        h,w = frame.shape[:2]
        cv2.putText(frame, f"Tracks: {n_tracks}", (w-130,20), cv2.FONT_HERSHEY_SIMPLEX, 0.52, (200,200,200), 1, cv2.LINE_AA)
