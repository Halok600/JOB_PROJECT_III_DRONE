...
class KalmanBoxFilter:
    ...
    def __init__(self):
        ...
        #measurement noise R (observation uncertainity)
        self.R = np.diag([1.0, 1.0, 10.0, 10.0]).astype(np.float32)
        # State covariance P
        self.P = np.diag([10.0, 10.0, 10.0, 10.0, 10000.0, 10000.0, 10000.0, 10000.0]).astype(np.float32)
        self.mean = np.zeros(8, dtype = np.float32)

    def initiate(self, bbox: np.ndarray) -> None:
        """Intialise state from first detection [cx, cy, w, h]."""
        self.mean[:4] = bbox.astype(np.float32)
        self.mean[4:] = 0.0
        #Scale intial covariance by bbox size
        std = max(bbox[2],bbox[3])
        
        self.P = np.diag([2*std, 2*std, 2*std, 2*std, 10*std, 10*std, 10*std, 10*std]).astype(np.float32)
