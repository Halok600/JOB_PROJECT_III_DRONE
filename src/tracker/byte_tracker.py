...
def _iou_matrix(tlbr_a: np.ndarray, tlbr_b: np.ndarray) -> np.ndarray:
    """Vectorised IoU between [x1,y1,x2,y2] box sets. Returns NxM matrix."""
...
class STrack:
    """Single object rack with Kalman state and trajectory history."""
    _id_counter: int = 0
...
class ByteTracker:
    ...
    def __init__(
        ...
        match_thresh: float = 0.80, #COST threshold: IoU >= 1-.80 = 0.20
    ...
    def update(self, detections: np.ndarray, warp: np.ndarray | None = None) -> list:
        ...
        m1,u_t1,u_d_high = _associate( self.tracked_stracks, list(high_dets),self.match_thresh)
        ...
        # NOTE: _ associate(tracks,detections,thres) - tracks first!
        ...
        m3, u_lost3, u_d_new = _associate(self.lost_stracks,list(unmatched_high),self.match_thresh)
        ...
        # --- Rebuild pools ---
        # Tracked: matched-in-stage1 + matched-in-stage2 + recovered_lost + new
        ...
        self.tracked_stracks = (stage1_matched + stage2_matched + recovered + new_stracks)

        # Lost newly lost + still-within-time-limit lost (not recovered)
        surviving_lost = [
            t for i, t in enumerate(self.lost_stracks)
            if i not in recovered_set and t.time_since_update <= self.max_time_lost
        ]
